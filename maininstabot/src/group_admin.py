...
========================================
    ...
    print("  !nc <base> <time> - Ultra-fast name rotation")
    print("  !ncstop - Stop name rotation")
    ...
    choice = input("\nChoose command (1-8): ").strip()

    if choice == "1":
        cmd = input("📝 Enter: !nc <base> <duration>\nExample: !nc CHU LOVERS 1m\n")
        result = handle_nc_command(cmd.replace("!nc ", ""), cl.user_id, "tester", thread_id, cl)
        print(f"\n📝 Result: {result}")

    elif choice == "2":
        result = handle_nc_stop_command(thread_id)
        print(f"\n📝 Result: {result}")
