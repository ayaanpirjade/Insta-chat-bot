...
def convert_image_to_mp4(image_path: str, output_video_path: str, duration_sec: int = 1) -> bool:
    """
    Converts a static image file into a 1-second H.264 encoded MP4 video.
    ...
    try:
        ...
        print(f"  ✅ Converted image to video: {output_video_path}")
        return True
...
def upload_media_to_dm(cl: Client, thread_id: str, file_path: str, caption: str = "") -> bool:
    ...
    Method 2: Video conversion bypass (when Instagram blocks image)
    Method 3: Direct send as file
    """
    ...
    # ── METHOD 2: Try direct photo upload ──
    try:
        time.sleep(0.5)
    ...
    # ── METHOD 3: Manual upload via private_request ──
    try:
    ...
    # ── METHOD 4: Convert to MP4 Video (Instagram Bypass) ──
    if CV2_AVAILABLE and PIL_AVAILABLE:
        try:
            ...
            if convert_image_to_mp4(abs_path, temp_video_path, duration_sec=1):
                time.sleep(1.0)
    ...
    # ── METHOD 5: Ultimate fallback - send as file ──
    try:
        time.sleep(1.0)
        cl.direct_send(abs_path, thread_ids=[thread_id])
        print("  ✅ Image sent as file")
        return True
    except Exception as e:
